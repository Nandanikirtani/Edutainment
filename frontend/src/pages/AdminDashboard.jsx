...
export default function AdminDashboard() {
  ...
  const fetchPendingVideos = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/pending`, {
        headers: { 
  ...
  const fetchRejectedVideos = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/rejected`, {
        headers: { 
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
  ...
  const fetchAdminUploadedCourses = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/admin/courses`, {
  ...
  const fetchAllCoursesWithStudents = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/admin/courses-with-students`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const fetchAllFaculty = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/admin/faculty`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const handleStatusChange = async (videoId, status) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/approve-reject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleUpdatePoints = async (courseId, studentId, points) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/admin/courses/${courseId}/students/${studentId}/points`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleUpdateCourseFaculty = async (courseId, facultyId) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/admin/courses/${courseId}/faculty`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleCourseUpload = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1'}/videos/courses/upload`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
        },
